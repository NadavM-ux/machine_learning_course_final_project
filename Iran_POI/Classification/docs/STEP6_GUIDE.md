...
> `person_vs_organization`) are trained and logged every iteration, in both balanced and
> unbalanced modes — **1,296 experiments** per iteration. Iterations are no longer
> hand-coded per notebook; they run through **`active_learning.py`**. For the live
> iteration-3→7 workflow, read **`ITERATIONS_HOWTO.md`** — it supersedes the
> iteration-2-specific parts below. The conceptual sections (uncertainty sampling,
> phases A/B/C) below are still accurate.
