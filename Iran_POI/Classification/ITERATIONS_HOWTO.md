...
> iteration-2-specific parts of `STEP6_GUIDE.md`. Two tasks only —
...
`locals_vs_diaspora` is gone from all modeling, plots, and the results CSVs.
(Your raw locals label files are kept untouched as an archive — nothing you
hand-collected was deleted.)
...
3. Fill the two empty columns per the tables above:
...
- runs all **432 experiments** (2 tasks × {3,2} classes × 9 feature sets × 6 algos ×
  {K-Fold, LOOCV} × **balanced=True only**) → `experiments_results_iteration_3.csv`,
...
|---|---|---|
| `mean_accuracy` / `mean_AUC` | average over **all 864** experiments | ⚠ blunt — dragged down by dozens of deliberately-bad configs. The PDF asks for the mean-accuracy plot, so keep it, but don't read quality into it. |
...
3. **2-class beats 3-class.** Dropping `unknown` (the 2-class runs) consistently scores
   ...
   a 2-class one.
4. **Always `balanced=True`** (instructor's rule — balanced is always better on this
   imbalanced data). The sweep now runs balanced-only; iterations 1–2 kept both versions
   on disk (per the original PDF) but every cross-iteration comparison and winner uses
   balanced=True, so it's consistent.
...
experiments_results_iteration_N.csv             ← Phase C, the 864-row sweep
```
